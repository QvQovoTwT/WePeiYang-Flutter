...
class PrivateChatProvider extends ChangeNotifier {
    ...
    try {
      ...
      if (result.isSuccess && result.data != null) {
        _contacts = (result.data as List).map((json) {
          ...
          final hasUserUnread = json.containsKey('user1_unread') ||
              json.containsKey('user2_unread');
  ...
  Future<void> selectContact(PrivateChatContact contact) async {
    ...
    _currentMessages = [];
    _totalUnreadCount = _contacts.fold<int>(0, (sum, c) => sum + c.unreadCount);
    notifyListeners();
  ...
  void _handleNewMessageSilent(PrivateChatMsgVO msg) {
    ...
    if (contactIndex == -1) {
      final contact = PrivateChatContact(
        userId: otherUserId,
        username:
            _userProfileCache[otherUserId]?['nickname'] ?? '用户 $otherUserId',
  ...
  void _updateContactLastMsgByUserId(int otherUserId, String lastMsg) {
    final matched = _contacts.where((c) => c.userId == otherUserId).toList();
    if (matched.isNotEmpty) {
  ...
  Future<String?> loadSetting() async {
    try {
      ...
      // 如果API失败，提供一个默认设置
      _userSetting = PrivateChatUserSetting(
        userId: _myUserId ?? 0,
        isEnable: 1,
        isAcceptStranger: 1,
        blockUserIds: '',
  ...
  Future<String?> toggleEnable(bool value) async {
    try {
      final result = await PrivateChatService.updateEnable(value ? 1 : 0);
      if (result.isSuccess) {
  ...
  Future<String?> toggleStranger(bool value) async {
    try {
      final result = await PrivateChatService.updateStranger(value ? 1 : 0);
      if (result.isSuccess) {
    ...
    try {
      ...
      if (result.isSuccess && result.data != null) {
        ...
        PrivateChatLogger.log(
            'Provider', '✅ 获取用户资料 uid=${contact.userId} nickname=$nickname');
      } else {
        PrivateChatLogger.log(
            'Provider', '⚠️ 获取用户资料失败 uid=${contact.userId}: ${result.msg}');
      }
    } catch (e) {
      PrivateChatLogger.log(
          'Provider', '⚠️ 获取用户资料异常 uid=${contact.userId}: $e');
    }
  }
  ...
  Future<void> refreshTotalUnreadCount() async {
    try {
      ...
      if (result.isSuccess && result.data is int) {
        ...
        final next = [serverCount, localCount, current]
